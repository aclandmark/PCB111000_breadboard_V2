...
{ int i = 14;                                               //Save characters starting from the end of the array rather than the begining
for(int m = 0; m <= 14; m++)s[m] = 0;
   do
  { *(s+i) = number % 10 + '0'; i-=1;  }
  while ((number = number / 10) > 0);
 
if (i >= 0){                                                //Shift the characters from the end of the array to the beginning
for(int m = 1; m <=(14-i); m++)*(s + m-1) = *(s + m+i); 
*(s + 14-i) = '.';}
 return (15-i);}
...
start_value += 1;
product = 1.0;}}
str_ptr = Int_to_String_B(result, result_as_string); 
